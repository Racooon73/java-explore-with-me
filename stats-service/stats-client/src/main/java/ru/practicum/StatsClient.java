...
public class StatsClient {
    ...
    public void hit(HttpServletRequest userRequest) {
        ...
        try {
            ...
            log.debug("Ответ от сервера статистики: {}", response);

        } catch (Exception e) {
            log.error("Статистика не записалась: ошибка", e);
        }
    ...
    public List<HitResponseDto> getStats(ViewsStatsRequest request) {
        try {
            ...
            log.error("Невозможно получить статистику для запроса: " + request, e);
        }
